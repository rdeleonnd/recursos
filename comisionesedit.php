...
$comisiones_edit = NULL; // Initialize page object first

class ccomisiones_edit extends ccomisiones {
	...
	// Page object name
	var $PageObjName = 'comisiones_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idcomisiones->CurrentValue == "")
			$this->Page_Terminate("comisioneslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->idcomisiones->FldIsDetailKey)
			$this->idcomisiones->setFormValue($objForm->GetValue("x_idcomisiones"));
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->idcomisiones->CurrentValue = $this->idcomisiones->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// inicio
			$this->inicio->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->inicio->CurrentValue, 11), ew_CurrentDate(), $this->inicio->ReadOnly);

			// fin
			$this->fin->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fin->CurrentValue, 11), ew_CurrentDate(), $this->fin->ReadOnly);
...
// Create page object
if (!isset($comisiones_edit)) $comisiones_edit = new ccomisiones_edit();

// Page init
$comisiones_edit->Page_Init();

// Page main
$comisiones_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$comisiones_edit->Page_Render();
?>
...
var CurrentForm = fcomisionesedit = new ew_Form("fcomisionesedit", "edit");

// Validate form
fcomisionesedit.Validate = function() {
...
fcomisionesedit.Form_CustomValidate = 
...
fcomisionesedit.ValidateRequired = true;
<?php } else { ?>
fcomisionesedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcomisionesedit.Lists["x_empleado_id"] = {"LinkField":"x_empleado_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_apellido","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $comisiones_edit->ShowPageHeader(); ?>
<?php
$comisiones_edit->ShowMessage();
?>
<form name="fcomisionesedit" id="fcomisionesedit" class="<?php echo $comisiones_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($comisiones_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $comisiones_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fcomisionesedit", "x_inicio", "%d/%m/%Y %H:%M:%S");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fcomisionesedit", "x_fin", "%d/%m/%Y %H:%M:%S");
</script>
<?php } ?>
</span>
...
<input type="hidden" data-table="comisiones" data-field="x_idcomisiones" name="x_idcomisiones" id="x_idcomisiones" value="<?php echo ew_HtmlEncode($comisiones->idcomisiones->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $comisiones_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fcomisionesedit.Init();
</script>
<?php
$comisiones_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$comisiones_edit->Page_Terminate();
?>
