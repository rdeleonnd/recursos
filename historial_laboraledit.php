...
$historial_laboral_edit = NULL; // Initialize page object first

class chistorial_laboral_edit extends chistorial_laboral {
	...
	// Page object name
	var $PageObjName = 'historial_laboral_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->historial_laboral_id->CurrentValue == "")
			$this->Page_Terminate("historial_laborallist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->historial_laboral_id->FldIsDetailKey)
			$this->historial_laboral_id->setFormValue($objForm->GetValue("x_historial_laboral_id"));
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->historial_laboral_id->CurrentValue = $this->historial_laboral_id->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// puesto
			$this->puesto->SetDbValueDef($rsnew, $this->puesto->CurrentValue, "", $this->puesto->ReadOnly);

			// atribuciones
			$this->atribuciones->SetDbValueDef($rsnew, $this->atribuciones->CurrentValue, "", $this->atribuciones->ReadOnly);

			// jefe
			$this->jefe->SetDbValueDef($rsnew, $this->jefe->CurrentValue, "", $this->jefe->ReadOnly);

			// fecha_ingreso
			$this->fecha_ingreso->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_ingreso->CurrentValue, 7), ew_CurrentDate(), $this->fecha_ingreso->ReadOnly);

			// fecha_egreso
			$this->fecha_egreso->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_egreso->CurrentValue, 7), ew_CurrentDate(), $this->fecha_egreso->ReadOnly);

			// sueldo_inicial
			$this->sueldo_inicial->SetDbValueDef($rsnew, $this->sueldo_inicial->CurrentValue, 0, $this->sueldo_inicial->ReadOnly);

			// sueldo_final
			$this->sueldo_final->SetDbValueDef($rsnew, $this->sueldo_final->CurrentValue, 0, $this->sueldo_final->ReadOnly);

			// motivo_retiro
			$this->motivo_retiro->SetDbValueDef($rsnew, $this->motivo_retiro->CurrentValue, "", $this->motivo_retiro->ReadOnly);
...
// Create page object
if (!isset($historial_laboral_edit)) $historial_laboral_edit = new chistorial_laboral_edit();

// Page init
$historial_laboral_edit->Page_Init();

// Page main
$historial_laboral_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$historial_laboral_edit->Page_Render();
?>
...
var CurrentForm = fhistorial_laboraledit = new ew_Form("fhistorial_laboraledit", "edit");

// Validate form
fhistorial_laboraledit.Validate = function() {
...
fhistorial_laboraledit.Form_CustomValidate = 
...
fhistorial_laboraledit.ValidateRequired = true;
<?php } else { ?>
fhistorial_laboraledit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fhistorial_laboraledit.Lists["x_empleado_id"] = {"LinkField":"x_empleado_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_apellido","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $historial_laboral_edit->ShowPageHeader(); ?>
<?php
$historial_laboral_edit->ShowMessage();
?>
<form name="fhistorial_laboraledit" id="fhistorial_laboraledit" class="<?php echo $historial_laboral_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($historial_laboral_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $historial_laboral_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fhistorial_laboraledit", "x_fecha_ingreso", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fhistorial_laboraledit", "x_fecha_egreso", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<input type="hidden" data-table="historial_laboral" data-field="x_historial_laboral_id" name="x_historial_laboral_id" id="x_historial_laboral_id" value="<?php echo ew_HtmlEncode($historial_laboral->historial_laboral_id->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $historial_laboral_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fhistorial_laboraledit.Init();
</script>
<?php
$historial_laboral_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$historial_laboral_edit->Page_Terminate();
?>
