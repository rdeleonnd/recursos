...
$vehiculo_edit = NULL; // Initialize page object first

class cvehiculo_edit extends cvehiculo {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->vehiculo_id->CurrentValue == "")
			$this->Page_Terminate("vehiculolist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->vehiculo_id->FldIsDetailKey)
			$this->vehiculo_id->setFormValue($objForm->GetValue("x_vehiculo_id"));
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->vehiculo_id->CurrentValue = $this->vehiculo_id->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// tipo_vehiculo_id
			$this->tipo_vehiculo_id->SetDbValueDef($rsnew, $this->tipo_vehiculo_id->CurrentValue, 0, $this->tipo_vehiculo_id->ReadOnly);

			// placas
			$this->placas->SetDbValueDef($rsnew, $this->placas->CurrentValue, "", $this->placas->ReadOnly);

			// modelo
			$this->modelo->SetDbValueDef($rsnew, $this->modelo->CurrentValue, 0, $this->modelo->ReadOnly);

			// color
			$this->color->SetDbValueDef($rsnew, $this->color->CurrentValue, "", $this->color->ReadOnly);

			// empleado_id
			$this->empleado_id->SetDbValueDef($rsnew, $this->empleado_id->CurrentValue, NULL, $this->empleado_id->ReadOnly);
...
// Create page object
if (!isset($vehiculo_edit)) $vehiculo_edit = new cvehiculo_edit();

// Page init
$vehiculo_edit->Page_Init();

// Page main
$vehiculo_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vehiculo_edit->Page_Render();
?>
...
var CurrentForm = fvehiculoedit = new ew_Form("fvehiculoedit", "edit");

// Validate form
fvehiculoedit.Validate = function() {
...
fvehiculoedit.Form_CustomValidate = 
...
fvehiculoedit.ValidateRequired = true;
<?php } else { ?>
fvehiculoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvehiculoedit.Lists["x_tipo_vehiculo_id"] = {"LinkField":"x_tipo_vehiculo_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_tipo","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvehiculoedit.Lists["x_empleado_id"] = {"LinkField":"x_empleado_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_apellido","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $vehiculo_edit->ShowPageHeader(); ?>
<?php
$vehiculo_edit->ShowMessage();
?>
<form name="fvehiculoedit" id="fvehiculoedit" class="<?php echo $vehiculo_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vehiculo_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vehiculo_edit->Token ?>">
<?php } ?>
...
<input type="hidden" data-table="vehiculo" data-field="x_vehiculo_id" name="x_vehiculo_id" id="x_vehiculo_id" value="<?php echo ew_HtmlEncode($vehiculo->vehiculo_id->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vehiculo_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
