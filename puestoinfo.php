...
class cpuesto extends cTable {
	...
	function __construct() {
		...
		// puesto_id
		$this->puesto_id = new cField('puesto', 'puesto', 'x_puesto_id', 'puesto_id', '`puesto_id`', '`puesto_id`', 3, -1, FALSE, '`puesto_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->puesto_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['puesto_id'] = &$this->puesto_id;

		// nombre
		$this->nombre = new cField('puesto', 'puesto', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['nombre'] = &$this->nombre;

		// estado
		$this->estado = new cField('puesto', 'puesto', 'x_estado', 'estado', '`estado`', '`estado`', 200, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['estado'] = &$this->estado;

		// seccion_id
		$this->seccion_id = new cField('puesto', 'puesto', 'x_seccion_id', 'seccion_id', '`seccion_id`', '`seccion_id`', 3, -1, FALSE, '`seccion_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('puesto_id', $rs))
				ew_AddFilter($where, ew_QuotedName('puesto_id', $this->DBID) . '=' . ew_QuotedValue($rs['puesto_id'], $this->puesto_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`puesto_id` = @puesto_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->puesto_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@puesto_id@", ew_AdjustSql($this->puesto_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "puesto_id:" . ew_VarToJson($this->puesto_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->puesto_id->CurrentValue)) {
			$sUrl .= "puesto_id=" . urlencode($this->puesto_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["puesto_id"]))
				$arKeys[] = ew_StripSlashes($_POST["puesto_id"]);
			elseif (isset($_GET["puesto_id"]))
				$arKeys[] = ew_StripSlashes($_GET["puesto_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->puesto_id->CurrentValue = $key;
   ...
   // Common render codes
		...
		$this->puesto_id->CellCssStyle = "white-space: nowrap;";
	...
	function RenderEditRow() {
		...
		$this->puesto_id->EditValue = $this->puesto_id->CurrentValue;
		$this->puesto_id->ViewCustomAttributes = "";
