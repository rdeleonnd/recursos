...
$vivienda_view = NULL; // Initialize page object first

class cvivienda_view extends cvivienda {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["vivienda_id"] <> "") {
			$this->RecKey["vivienda_id"] = $_GET["vivienda_id"];
			$KeyUrl .= "&amp;vivienda_id=" . urlencode($this->RecKey["vivienda_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["vivienda_id"] <> "") {
				...
				$this->RecKey["vivienda_id"] = $this->vivienda_id->QueryStringValue;
			} elseif (@$_POST["vivienda_id"] <> "") {
				$this->vivienda_id->setFormValue($_POST["vivienda_id"]);
				$this->RecKey["vivienda_id"] = $this->vivienda_id->FormValue;
			} else {
				$sReturnUrl = "viviendalist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "viviendalist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "viviendalist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($vivienda_view)) $vivienda_view = new cvivienda_view();

// Page init
$vivienda_view->Page_Init();

// Page main
$vivienda_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vivienda_view->Page_Render();
?>
...
var CurrentForm = fviviendaview = new ew_Form("fviviendaview", "view");

// Form_CustomValidate event
fviviendaview.Form_CustomValidate = 
...
fviviendaview.ValidateRequired = true;
<?php } else { ?>
fviviendaview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fviviendaview.Lists["x_tipo_vivienda_id"] = {"LinkField":"x_tipo_vivienda_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_vivienda","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fviviendaview.Lists["x_empleado_id"] = {"LinkField":"x_empleado_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_apellido","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $vivienda_view->ExportOptions->Render("body") ?>
<?php
	foreach ($vivienda_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $vivienda_view->ShowPageHeader(); ?>
<?php
$vivienda_view->ShowMessage();
?>
<form name="fviviendaview" id="fviviendaview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vivienda_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vivienda_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vivienda">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($vivienda->tipo_vivienda_id->Visible) { // tipo_vivienda_id ?>
	<tr id="r_tipo_vivienda_id">
		<td><span id="elh_vivienda_tipo_vivienda_id"><?php echo $vivienda->tipo_vivienda_id->FldCaption() ?></span></td>
...
<?php echo $vivienda->tipo_vivienda_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($vivienda->descripcion->Visible) { // descripcion ?>
	<tr id="r_descripcion">
		<td><span id="elh_vivienda_descripcion"><?php echo $vivienda->descripcion->FldCaption() ?></span></td>
...
<?php echo $vivienda->descripcion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($vivienda->empleado_id->Visible) { // empleado_id ?>
	<tr id="r_empleado_id">
		<td><span id="elh_vivienda_empleado_id"><?php echo $vivienda->empleado_id->FldCaption() ?></span></td>
...
<?php echo $vivienda->empleado_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
