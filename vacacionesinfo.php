...
//
// Table class for vacaciones
//
class cvacaciones extends cTable {
	var $vacaciones_id;
	var $fecha_inicio;
	var $fecha_fin;
	var $observaciones;
	...
	function __construct() {
		...
		$this->TableVar = 'vacaciones';
		$this->TableName = 'vacaciones';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`vacaciones`";
		$this->DBID = 'DB';
		...
		// vacaciones_id
		$this->vacaciones_id = new cField('vacaciones', 'vacaciones', 'x_vacaciones_id', 'vacaciones_id', '`vacaciones_id`', '`vacaciones_id`', 3, -1, FALSE, '`vacaciones_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->vacaciones_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['vacaciones_id'] = &$this->vacaciones_id;

		// fecha_inicio
		$this->fecha_inicio = new cField('vacaciones', 'vacaciones', 'x_fecha_inicio', 'fecha_inicio', '`fecha_inicio`', 'DATE_FORMAT(`fecha_inicio`, \'%d/%m/%Y\')', 133, 11, FALSE, '`fecha_inicio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// fecha_fin
		$this->fecha_fin = new cField('vacaciones', 'vacaciones', 'x_fecha_fin', 'fecha_fin', '`fecha_fin`', 'DATE_FORMAT(`fecha_fin`, \'%d/%m/%Y\')', 133, 11, FALSE, '`fecha_fin`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fecha_fin->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha_fin'] = &$this->fecha_fin;

		// observaciones
		$this->observaciones = new cField('vacaciones', 'vacaciones', 'x_observaciones', 'observaciones', '`observaciones`', '`observaciones`', 200, -1, FALSE, '`observaciones`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['observaciones'] = &$this->observaciones;

		// empleado_id
		$this->empleado_id = new cField('vacaciones', 'vacaciones', 'x_empleado_id', 'empleado_id', '`empleado_id`', '`empleado_id`', 3, -1, FALSE, '`empleado_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`vacaciones`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('vacaciones_id', $rs))
				ew_AddFilter($where, ew_QuotedName('vacaciones_id', $this->DBID) . '=' . ew_QuotedValue($rs['vacaciones_id'], $this->vacaciones_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`vacaciones_id` = @vacaciones_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->vacaciones_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@vacaciones_id@", ew_AdjustSql($this->vacaciones_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("vacacionesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("vacacionesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "vacacionesadd.php?" . $this->UrlParm($parm);
		else
			$url = "vacacionesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("vacacionesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("vacacionesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("vacacionesdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "vacaciones_id:" . ew_VarToJson($this->vacaciones_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->vacaciones_id->CurrentValue)) {
			$sUrl .= "vacaciones_id=" . urlencode($this->vacaciones_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["vacaciones_id"]))
				$arKeys[] = ew_StripSlashes($_POST["vacaciones_id"]);
			elseif (isset($_GET["vacaciones_id"]))
				$arKeys[] = ew_StripSlashes($_GET["vacaciones_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->vacaciones_id->CurrentValue = $key;
   ...
   // Common render codes
		...
		$this->vacaciones_id->ViewValue = $this->vacaciones_id->CurrentValue;
		$this->vacaciones_id->ViewCustomAttributes = "";
		...
		// vacaciones_id
		$this->vacaciones_id->LinkCustomAttributes = "";
		$this->vacaciones_id->HrefValue = "";
		$this->vacaciones_id->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// vacaciones_id
		$this->vacaciones_id->EditAttrs["class"] = "form-control";
		$this->vacaciones_id->EditCustomAttributes = "";
		$this->vacaciones_id->EditValue = $this->vacaciones_id->CurrentValue;
		$this->vacaciones_id->ViewCustomAttributes = "";
