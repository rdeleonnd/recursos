...
$vehiculo_view = NULL; // Initialize page object first

class cvehiculo_view extends cvehiculo {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["vehiculo_id"] <> "") {
			$this->RecKey["vehiculo_id"] = $_GET["vehiculo_id"];
			$KeyUrl .= "&amp;vehiculo_id=" . urlencode($this->RecKey["vehiculo_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["vehiculo_id"] <> "") {
				...
				$this->RecKey["vehiculo_id"] = $this->vehiculo_id->QueryStringValue;
			} elseif (@$_POST["vehiculo_id"] <> "") {
				$this->vehiculo_id->setFormValue($_POST["vehiculo_id"]);
				$this->RecKey["vehiculo_id"] = $this->vehiculo_id->FormValue;
			} else {
				$sReturnUrl = "vehiculolist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "vehiculolist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "vehiculolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($vehiculo_view)) $vehiculo_view = new cvehiculo_view();

// Page init
$vehiculo_view->Page_Init();

// Page main
$vehiculo_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vehiculo_view->Page_Render();
?>
...
var CurrentForm = fvehiculoview = new ew_Form("fvehiculoview", "view");

// Form_CustomValidate event
fvehiculoview.Form_CustomValidate = 
...
fvehiculoview.ValidateRequired = true;
<?php } else { ?>
fvehiculoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvehiculoview.Lists["x_tipo_vehiculo_id"] = {"LinkField":"x_tipo_vehiculo_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_tipo","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvehiculoview.Lists["x_empleado_id"] = {"LinkField":"x_empleado_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_apellido","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $vehiculo_view->ExportOptions->Render("body") ?>
<?php
	foreach ($vehiculo_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $vehiculo_view->ShowPageHeader(); ?>
<?php
$vehiculo_view->ShowMessage();
?>
<form name="fvehiculoview" id="fvehiculoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vehiculo_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vehiculo_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vehiculo">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($vehiculo->tipo_vehiculo_id->Visible) { // tipo_vehiculo_id ?>
	<tr id="r_tipo_vehiculo_id">
		<td><span id="elh_vehiculo_tipo_vehiculo_id"><?php echo $vehiculo->tipo_vehiculo_id->FldCaption() ?></span></td>
...
<?php echo $vehiculo->tipo_vehiculo_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($vehiculo->placas->Visible) { // placas ?>
	<tr id="r_placas">
		<td><span id="elh_vehiculo_placas"><?php echo $vehiculo->placas->FldCaption() ?></span></td>
...
<?php echo $vehiculo->placas->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($vehiculo->modelo->Visible) { // modelo ?>
	<tr id="r_modelo">
		<td><span id="elh_vehiculo_modelo"><?php echo $vehiculo->modelo->FldCaption() ?></span></td>
...
<?php echo $vehiculo->modelo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($vehiculo->color->Visible) { // color ?>
	<tr id="r_color">
		<td><span id="elh_vehiculo_color"><?php echo $vehiculo->color->FldCaption() ?></span></td>
...
<?php echo $vehiculo->color->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($vehiculo->empleado_id->Visible) { // empleado_id ?>
	<tr id="r_empleado_id">
		<td><span id="elh_vehiculo_empleado_id"><?php echo $vehiculo->empleado_id->FldCaption() ?></span></td>
...
<?php echo $vehiculo->empleado_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
