	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["permisos_id"] != "") {
				...
				$this->setKey("permisos_id", $this->permisos_id->CurrentValue); // Set up key
			} else {
				$this->setKey("permisos_id", ""); // Clear key
	...
	function AddRow($rsold = NULL) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->permisos_id->setDbValue($conn->Insert_ID());
				$rsnew['permisos_id'] = $this->permisos_id->DbValue;
			}
		} else {
...
fpermisosadd.Lists["x_empleado_id"] = {"LinkField":"x_empleado_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_apellido","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<script type="text/javascript">
ew_CreateCalendar("fpermisosadd", "x_fecha_inicio", "%d/%m/%Y %H:%M:%S");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fpermisosadd", "x_fecha_fin", "%d/%m/%Y %H:%M:%S");
</script>
<?php } ?>
</span>
