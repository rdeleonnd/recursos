...
class csede_add extends csede {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["sede_id"] != "") {
				...
				$this->setKey("sede_id", $this->sede_id->CurrentValue); // Set up key
			} else {
				$this->setKey("sede_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "sedeview.php")
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("sede_id")) <> "")
			$this->sede_id->CurrentValue = $this->getKey("sede_id"); // sede_id
		else
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->sede_id->setDbValue($conn->Insert_ID());
				$rsnew['sede_id'] = $this->sede_id->DbValue;
			}
		} else {
...
var CurrentForm = fsedeadd = new ew_Form("fsedeadd", "add");
...
fsedeadd.Lists["x_departamento_id"] = {"LinkField":"x_departamento_origen_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
?>
<form name="fsedeadd" id="fsedeadd" class="<?php echo $sede_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($sede_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $sede_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $sede_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
