...
class cuserpriv extends cuserlevel {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["userlevel"])) $GLOBALS["userlevel"] = &$this;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'userlevel');
	...
	function Page_Main() {
		...
		$Breadcrumb->Add("list", "userlevel", "userlevellist.php", "", "userlevel");
		...
		if (@$_POST["a_edit"] == "") {
			...
			if (@$_GET["userlevelid"] <> "") {
				...
				$this->Page_Terminate("userlevellist.php"); // Return to list
			}
...
<p><?php echo $Language->Phrase("UserLevel") ?><?php echo $Security->GetUserLevelName($userlevel->userlevelid->CurrentValue) ?>(<?php echo $userlevel->userlevelid->CurrentValue ?>)</p>
<?php
...
<input type="hidden" name="x_userlevelid" id="x_userlevelid" value="<?php echo $userlevel->userlevelid->CurrentValue ?>">
...
for ($i = 0; $i < $userpriv->TableNameCount; $i++) {
	$userpriv->TempPriv = $Security->GetUserLevelPrivEx($userpriv->TableList[$i][4] . $userpriv->TableList[$i][0], $userlevel->userlevelid->CurrentValue);
