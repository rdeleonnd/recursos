...
class cpuesto_list extends cpuesto {
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->puesto_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->puesto_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->seccion_id->AdvancedSearch->ToJSON(), ","); // Field seccion_id
	...
	function RestoreFilterList() {
		...
		// Field seccion_id
		$this->seccion_id->AdvancedSearch->SearchValue = @$filter["x_seccion_id"];
		$this->seccion_id->AdvancedSearch->SearchOperator = @$filter["z_seccion_id"];
		$this->seccion_id->AdvancedSearch->SearchCondition = @$filter["v_seccion_id"];
		$this->seccion_id->AdvancedSearch->SearchValue2 = @$filter["y_seccion_id"];
		$this->seccion_id->AdvancedSearch->SearchOperator2 = @$filter["w_seccion_id"];
		$this->seccion_id->AdvancedSearch->Save();
	}
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->seccion_id, $Default, FALSE); // seccion_id
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->seccion_id->AdvancedSearch->Save(); // seccion_id
		}
		return $sWhere;
	}
	...
	function CheckSearchParms() {
		...
		if ($this->estado->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->seccion_id->AdvancedSearch->IssetSession())
			return TRUE;
		return FALSE;
	}
	...
	function ResetAdvancedSearchParms() {
		...
		$this->seccion_id->AdvancedSearch->UnsetSession();
	}
	...
	function RestoreSearchParms() {
		...
		$this->seccion_id->AdvancedSearch->Load();
	}
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->puesto_id->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpuestolistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadSearchValues() {
		...
		// seccion_id
		$this->seccion_id->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_seccion_id"]);
		if ($this->seccion_id->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->seccion_id->AdvancedSearch->SearchOperator = @$_GET["z_seccion_id"];
	}
	...
	function RenderRow() {
		...
		$this->seccion_id->ViewCustomAttributes = "";
			...
			if (trim(strval($this->seccion_id->AdvancedSearch->SearchValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`seccion_id`" . ew_SearchString("=", $this->seccion_id->AdvancedSearch->SearchValue, EW_DATATYPE_NUMBER, "");
			}
...
fpuestolist.Lists["x_seccion_id"] = {"LinkField":"x_seccion_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};

// Form object for search
var CurrentSearchForm = fpuestolistsrch = new ew_Form("fpuestolistsrch");

// Validate function for search
fpuestolistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
fpuestolistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
<?php if (EW_CLIENT_VALIDATE) { ?>
fpuestolistsrch.ValidateRequired = true; // Use JavaScript validation
<?php } else { ?>
fpuestolistsrch.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Dynamic selection lists
fpuestolistsrch.Lists["x_seccion_id"] = {"LinkField":"x_seccion_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($puesto_list->SearchOptions->Visible()) { ?>
<?php $puesto_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($puesto_list->FilterOptions->Visible()) { ?>
<?php $puesto_list->FilterOptions->Render("body") ?>
<?php } ?>
...
<?php
	...
	if ($puesto->CurrentAction == "" && $puesto_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$puesto_list->setWarningMessage(ew_DeniedMsg());
...
<?php if ($puesto->Export == "" && $puesto->CurrentAction == "") { ?>
<form name="fpuestolistsrch" id="fpuestolistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($puesto_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fpuestolistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="puesto">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$puesto_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$puesto->RowType = EW_ROWTYPE_SEARCH;
...
<?php if ($puesto->nombre->Visible) { // nombre ?>
	<div id="xsc_nombre" class="ewCell form-group">
		<label for="x_nombre" class="ewSearchCaption ewLabel"><?php echo $puesto->nombre->FldCaption() ?></label>
...
<?php if ($puesto->seccion_id->Visible) { // seccion_id ?>
	<div id="xsc_seccion_id" class="ewCell form-group">
		<label for="x_seccion_id" class="ewSearchCaption ewLabel"><?php echo $puesto->seccion_id->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_seccion_id" id="z_seccion_id" value="="></span>
		<span class="ewSearchField">
...
if (is_array($puesto->seccion_id->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($puesto->seccion_id->AdvancedSearch->SearchValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
...
?>
<?php if ($puesto->nombre->Visible) { // nombre ?>
	<?php if ($puesto->SortUrl($puesto->nombre) == "") { ?>
		<th data-name="nombre"><div id="elh_puesto_nombre" class="puesto_nombre"><div class="ewTableHeaderCaption"><?php echo $puesto->nombre->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nombre"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $puesto->SortUrl($puesto->nombre) ?>',1);"><div id="elh_puesto_nombre" class="puesto_nombre">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $puesto->nombre->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($puesto->nombre->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($puesto->nombre->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($puesto->estado->Visible) { // estado ?>
	<?php if ($puesto->SortUrl($puesto->estado) == "") { ?>
		<th data-name="estado"><div id="elh_puesto_estado" class="puesto_estado"><div class="ewTableHeaderCaption"><?php echo $puesto->estado->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="estado"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $puesto->SortUrl($puesto->estado) ?>',1);"><div id="elh_puesto_estado" class="puesto_estado">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $puesto->estado->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($puesto->estado->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($puesto->estado->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($puesto->seccion_id->Visible) { // seccion_id ?>
	<?php if ($puesto->SortUrl($puesto->seccion_id) == "") { ?>
		<th data-name="seccion_id"><div id="elh_puesto_seccion_id" class="puesto_seccion_id"><div class="ewTableHeaderCaption"><?php echo $puesto->seccion_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="seccion_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $puesto->SortUrl($puesto->seccion_id) ?>',1);"><div id="elh_puesto_seccion_id" class="puesto_seccion_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $puesto->seccion_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($puesto->seccion_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($puesto->seccion_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<span id="el<?php echo $puesto_list->RowCnt ?>_puesto_nombre" class="puesto_nombre">
<span<?php echo $puesto->nombre->ViewAttributes() ?>>
<?php echo $puesto->nombre->ListViewValue() ?></span>
</span>
...
<span id="el<?php echo $puesto_list->RowCnt ?>_puesto_estado" class="puesto_estado">
<span<?php echo $puesto->estado->ViewAttributes() ?>>
<?php echo $puesto->estado->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($puesto->seccion_id->Visible) { // seccion_id ?>
		<td data-name="seccion_id"<?php echo $puesto->seccion_id->CellAttributes() ?>>
<span id="el<?php echo $puesto_list->RowCnt ?>_puesto_seccion_id" class="puesto_seccion_id">
<span<?php echo $puesto->seccion_id->ViewAttributes() ?>>
<?php echo $puesto->seccion_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
<?php } ?>
<script type="text/javascript">
fpuestolistsrch.Init();
fpuestolistsrch.FilterList = <?php echo $puesto_list->GetFilterList() ?>;
fpuestolist.Init();
</script>
<?php
