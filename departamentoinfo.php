...
class cdepartamento extends cTable {
	...
	function __construct() {
		...
		// departamento_id
		$this->departamento_id = new cField('departamento', 'departamento', 'x_departamento_id', 'departamento_id', '`departamento_id`', '`departamento_id`', 3, -1, FALSE, '`departamento_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// estado
		$this->estado = new cField('departamento', 'departamento', 'x_estado', 'estado', '`estado`', '`estado`', 200, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['estado'] = &$this->estado;

		// area_id
		$this->area_id = new cField('departamento', 'departamento', 'x_area_id', 'area_id', '`area_id`', '`area_id`', 3, -1, FALSE, '`area_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('departamento_id', $rs))
				ew_AddFilter($where, ew_QuotedName('departamento_id', $this->DBID) . '=' . ew_QuotedValue($rs['departamento_id'], $this->departamento_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->departamento_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@departamento_id@", ew_AdjustSql($this->departamento_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "departamento_id:" . ew_VarToJson($this->departamento_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->departamento_id->CurrentValue)) {
			$sUrl .= "departamento_id=" . urlencode($this->departamento_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["departamento_id"]))
				$arKeys[] = ew_StripSlashes($_POST["departamento_id"]);
			elseif (isset($_GET["departamento_id"]))
				$arKeys[] = ew_StripSlashes($_GET["departamento_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->departamento_id->CurrentValue = $key;
