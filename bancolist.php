...
class cbanco_list extends cbanco {
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->banco_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->banco_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->banco_id->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("banco_id")) <> "")
			$this->banco_id->CurrentValue = $this->getKey("banco_id"); // banco_id
		else
			$bValidKey = FALSE;
...
<?php if ($banco->nombre->Visible) { // nombre ?>
	...
	<?php } else { ?>
		<th data-name="nombre"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $banco->SortUrl($banco->nombre) ?>',1);"><div id="elh_banco_nombre" class="banco_nombre">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $banco->nombre->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($banco->nombre->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($banco->nombre->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
